...
export default function AnimalGallery({ animal }) {
  ...
  const selectedPhoto = photos[selected] || photos[0] || "/logo.png";

  return (
    <div className="animal-gallery">
      <div className="animal-gallery-main">
        <img
          src={mediaUrl(selectedPhoto, { width: 1200, height: 1000, crop: "fill" })}
          alt={`${animal.name} - foto ${selected + 1}`}
          loading="eager"
          fetchPriority="high"
          decoding="async"
        />
        {photos.length > 1 && <span className="gallery-counter">{selected + 1} / {photos.length}</span>}
