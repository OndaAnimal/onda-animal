...
export function analyzeAnimalWithAi(photos) {
  return apiJson("/api/admin/ai", {
    method: "POST",
    body: JSON.stringify({ action: "analyze", photos }),
  });
}

export function composeAnimalTextsWithAi(animal) {
  return apiJson("/api/admin/ai", {
    method: "POST",
    body: JSON.stringify({ action: "compose", animal }),
  });
}

export function rewriteAnimalTextWithAi(field, animal, currentText = "") {
  return apiJson("/api/admin/ai", {
    method: "POST",
    body: JSON.stringify({ action: "rewrite", field, animal, currentText }),
  });
}
